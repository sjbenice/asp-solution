...
namespace BusinessLayer.Pages
{
	public class QuotationWorkOrderListDB : DBBase<QuotationWorkOrderList, List<QuotationWorkOrderList>, long>
	{
		...
		public List<QuotationWorkOrderList> GetByMasterIDFromSession(long masterId, long mainMasterId)
		{
			...
			if (obj == null)
			{
				list = ((mainMasterId <= 0 || masterId <= 0) ? new List<QuotationWorkOrderList>() : ((IQueryable<QuotationWorkOrderList>)dbContext.QuotationWorkOrderList).Where((QuotationWorkOrderList j) => j.FkQuotationDetailsID == (long?)masterId).Take(1).ToList());
			}
			else
			{
				list = obj as List<QuotationWorkOrderList>;
				...
				{
					list.AddRange(((IQueryable<QuotationWorkOrderList>)dbContext.QuotationWorkOrderList).Where((QuotationWorkOrderList j) => j.FkQuotationDetailsID == (long?)masterId).Take(1).ToList());
				}
			}
			...
			HttpContext.Current.Session["QuotationWorkOrderList"] = list;
			return list.Where((QuotationWorkOrderList x) => x.FkQuotationDetailsID == masterId && x.RowStatus != 3).Take(1).ToList();
		}
		...
		public QuotationWorkOrderList GetByIDFromSession(long id)
		{
			...
			return source.FirstOrDefault((QuotationWorkOrderList j) => j.QuotationWorkOrderID == id);
		}
		...
		public bool UpdateDetailsFromSession()
		{
			...
			if (obj != null)
			{
				...
				foreach (QuotationWorkOrderList item in source.Where((QuotationWorkOrderList x) => x.RowStatus == 1).ToList())
				{
					Insert(item);
				}
				foreach (QuotationWorkOrderList item2 in source.Where((QuotationWorkOrderList x) => x.RowStatus == 2).ToList())
				{
					Update(item2);
				}
