...
namespace BusinessLayer.Pages
{
	public class RSSMasterDB : DBBase<RSSMaster, List<RSSMaster>, long>
	{
		...
		public override bool Insert(RSSMaster entity, out string message)
		{
			...
			try
			{
				...
				if (((DbContext)dbContext).SaveChanges() > 0)
				{
                    ...
                    context.Session["RSSMasterId"] = entity.RSSMasterId;
                    return true;
				}
				message = "InsertError";
				return false;
        ...
        public bool RemoveRSS(RSSMaster entity, string reasonForDelete, out string message)
        {
            message = "";
            try
            {
                entity.IsSampled = true;
                entity.IsDeleted = true;
                entity.ReasonForDelete = reasonForDelete;
                ...
                ((DbContext)dbContext).Entry<RSSMaster>(entity).State = (EntityState)16;
                if (((DbContext)dbContext).SaveChanges() > 0)
                {
                    return true;
                }
		...
		{
            //return ((IEnumerable<ViewRSS>)dbContext.ViewRSS).ToList();
