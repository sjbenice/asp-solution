	...
	public class ValidityDetailsReports : Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(text) && this.cmbValidityName.Value != null)
			{
                try
                {
                    string notInUsed = ckNotInUsedEquipment.Checked ? " ([NotInUsed] =true or  [NotInUsed] = false)" : "[NotInUsed]=false";
                    this.ReportViewer1.Report = this.GetReport(text, (int)this.cmbValidityName.Value,  notInUsed);
                }
                catch (Exception ex)
                {
		...
		protected void btnGenerate_Click(object sender, EventArgs e)
		{
			...
			if (a == "Valid")
			{
				this.ShowReport("ValidValidityDetailsReport", id,  notInUsed);
				return;
			}
			else if (a == "Expired")
			{
				this.ShowReport("ExpiredValidityDetailsReport", id, notInUsed);
				return;
			}
			else if (a == "NotInUsed")
			{
				this.ShowReport("NotInUsedValidityDetailsReport", id, "1=1");
				return;
			}
			else if (a == "Late")
			{
				this.ShowReport("LateValidityDetailsReport", id, notInUsed);
            }
            else
            {
                this.ShowReport("AllValidityDetailsReport", id, notInUsed);
            }
		}
		...
		protected XtraReport GetReport(string Report, int id, string FilterExpression)
		{
			...
			if (Report == "ValidValidityDetailsReport")
			{
                ...
                validValidityDetailsReport.Parameters["FilterExpression"].Value = FilterExpression;
			...
			if (Report == "ExpiredValidityDetailsReport")
			{
                ...
                expiredValidityDetailsReport.Parameters["FilterExpression"].Value = FilterExpression;

                expiredValidityDetailsReport.CreateDocument();
				return expiredValidityDetailsReport;
			}
			if (Report == "NotInUsedValidityDetailsReport")
			{
				NotInUsedValidityDetailsReport notInUsedValidityDetailsReport = new NotInUsedValidityDetailsReport();
				notInUsedValidityDetailsReport.Parameters["Id"].Value = id;
                notInUsedValidityDetailsReport.Parameters["FilterExpression"].Value = "1=1";

                notInUsedValidityDetailsReport.CreateDocument();
				return notInUsedValidityDetailsReport;
			}
			if (Report == "LateValidityDetailsReport")
			{
                ...
                lateValidityDetailsReport.Parameters["FilterExpression"].Value = FilterExpression;
            ...
            if (Report == "AllValidityDetailsReport")
            {
                AllValidityDetailsReport validityDetailsReport = new AllValidityDetailsReport();
                validityDetailsReport.Parameters["Id"].Value = id;
        ...
        protected ASPxCheckBox ckNotInUsedEquipment;
