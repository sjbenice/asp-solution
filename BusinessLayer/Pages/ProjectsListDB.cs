...
namespace BusinessLayer.Pages
{
	public class ProjectsListDB : DBBase<ProjectsList, List<ProjectsList>, int>
	{
			...
			try
			{
				if (!CheckExist(entity.AshghalCode) || entity.AshghalCode == "")
				{
					...
					if (((DbContext)dbContext).SaveChanges() > 0)
					{
                        ...
                        context.Session["ProjectID"] = entity.ProjectID;
                        return true;
					}
					message = "InsertError";
					return false;
        ...
        // Created By Barkat to get the only active projectList
        public List<ProjectsList> GetActiveProjectList()
        {
            var dt = ((IEnumerable<ProjectsList>)dbContext.ProjectsList.Where(x => x.IsClosed == false)).ToList();
			return dt;
        }
