	...
	public class JobOrderDetailsDB : DBBase<JobOrderDetails, List<JobOrderDetails>, long>
	{
		...
		public bool UpdateDetailsFromSession(long JobOrderMasterID)
		{
			...
			if (obj != null)
			{
				...
				foreach (JobOrderDetails entity in source.Where((JobOrderDetails x) => x.RowStatus == 1).ToList())
				{
        ...
        public decimal GetNotReceiveTestsByJobOrderMasterIDTestID(long JobOrderMasterID, List<int> selected)
        {
            List<JobOrderDetails> list = ((IQueryable<JobOrderDetails>)dbContext.JobOrderDetails).Where((JobOrderDetails j) => j.FKJobOrderMasterID == JobOrderMasterID).ToList();
            decimal result = 0m;
            foreach (JobOrderDetails jobOrder in list)
            {
                if (selected.Count == 0 || !selected.Contains(jobOrder.FKTestID))
                    continue;

                if (jobOrder.Price.HasValue)
                {
                    result += jobOrder.Price.Value * jobOrder.MinQty.GetValueOrDefault(0);
                }
            }
            return result;
        }
