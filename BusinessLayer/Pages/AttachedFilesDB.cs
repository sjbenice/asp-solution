...
namespace BusinessLayer.Pages
{
	public class AttachedFilesDB : DBBase<AttachedFiles, List<AttachedFiles>, long>
	{
        ...
        public int CountByReportNumber(string reportnumber, int transTypeID)
        {
            var count = dbContext.AttachedFiles
                .Where(entity => entity.FKTransTypeID == transTypeID && entity.FileName.Contains(reportnumber))
                .Count();
	 ...
	 public string GetScannedLabWs(long transID, int transTypeID)
        {
            var count = dbContext.AttachedFiles
                .Where(entity => entity.FKTransID == transID && entity.FKTransTypeID == transTypeID)
                .Count();

            if (count > 0)
            {
                // There are matching records, concatenate them into a comma-separated single row
                var scannedLabWs = dbContext.AttachedFiles
                    .Where(entity => entity.FKTransID == transID && entity.FKTransTypeID == transTypeID)
                    .Select(entity => entity.FileName)
                    .ToList();

                return string.Join(",", scannedLabWs);
            }
            else
            {
                // No matching records
                return ""; // or return an empty string, depending on your requirements
            }
        }																
        ...
        public AttachedFiles GetAttachment(long transID, int transTypeID)
        {
            AttachedFiles attachedFiles = ((IQueryable<AttachedFiles>)dbContext.AttachedFiles).Where((AttachedFiles e) => e.FKTransID == transID && e.FKTransTypeID == transTypeID).ToList().FirstOrDefault();
            return attachedFiles;
        }

        public List<AttachedFiles> GetAttachmentListListByTransID(long transID)
        {
            List<AttachedFiles> attachedFilesList = dbContext.AttachedFiles
                .Where(e => e.FKTransID == transID)
                .ToList();
