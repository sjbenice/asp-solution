...
namespace BusinessLayer.Pages
{
	public class TestsListDB : DBBase<TestsList, List<TestsList>, int>
	{
		...
		public List<TestsList> GetAllActive()
		{
			return ((IEnumerable<TestsList>)dbContext.TestsList).Where(c => !c.IsLocked).ToList();
		}

		public List<TestsList> GetAllWithoutGroup(int FKMaterialDetailsID)
        {
            List<TestsList> testList = new List<TestsList>();
            object obj = HttpContext.Current.Session["TestsList"];
            if (obj != null && FKMaterialDetailsID != 0)
                testList = HttpContext.Current.Session["TestsList"] as List<TestsList>;
            else
            {
                List<MaterialsDetailsTests> materialsDetailsTestsList = dbContext.MaterialsDetailsTests.Where(x => x.FKMaterialDetailsID == FKMaterialDetailsID).ToList();
                foreach (TestsList test in dbContext.TestsList.Where(t => t.IsLocked == false).ToList())
                {
                    MaterialsDetailsTests materialsDetailsTests = materialsDetailsTestsList.Where(r => r.FKTestID == test.TestID).FirstOrDefault();
                    if (materialsDetailsTests != null)
                        test.FkGroupId = materialsDetailsTests.FKGroupID;
                    else
                        test.FkGroupId = null;
                ...
                HttpContext.Current.Session["TestsList"] = testList;
            }
        ...
        public bool UpdateToSession(TestsList entity)
        {
            object obj = HttpContext.Current.Session["TestsList"];
            if (obj != null)
            {
                List<TestsList> list = obj as List<TestsList>;
                TestsList sampleReceiveTestList = list.First((TestsList x) => x.TestID == entity.TestID);
                sampleReceiveTestList.FkGroupId = entity.FkGroupId;
                HttpContext.Current.Session["TestsList"] = list;
            }
            return true;
        }
    }
