...
namespace BusinessLayer.Pages
{
	public class SampleReceiveTestListDB : DBBase<SampleReceiveTestList, List<SampleReceiveTestList>, long>
	{
        ...
        public List<SampleReceiveTestList> GetReport()
        {
            return ((IEnumerable<SampleReceiveTestList>)dbContext.SampleReceiveTestList).Where(item => item.ReportNumber != null).GroupBy(item => item.ReportNumber)
        ...
        public List<SampleReceiveTestList> GetByReportNumber(int reportNumber)
        {
            return dbContext.SampleReceiveTestList.Where(s => s.ReportNumber == reportNumber).ToList();
        }
		...
		public override bool Delete(SampleReceiveTestList entity, out string message)
		{
			...
			try
			{
                var testReportings = (IQueryable<TestReporting>)dbContext.TestReportings.Where((TestReporting t) => t.FKSampleReceiveTestID == entity.SampleReceiveTestID);
                foreach(TestReporting testReporting in testReportings)
                    this.dbContext.Entry<TestReporting>(testReporting).State = EntityState.Deleted;
		...
		public List<SampleReceiveTestList> GetByMasterIDFromSession(long masterId, long JobOrderMasterID, int? MaterialTypeID, int? MaterialDetailsID)
		{
			if (MaterialDetailsID.HasValue)
			{
				...
				if (obj == null)
				{
                    ...
                    var sampleservice = new SampleReceiveListDB();
					foreach (SampleReceiveTestList item2 in result)
					{
						if (sampleservice.IsInvoiced(masterId)/*item2.IsApproved == true || item2.IsInvoiced*/)
						{
                        ...
                        JobOrderDetails jobOrderDet = byMasterIDWithSession.FirstOrDefault((JobOrderDetails x) => x.FKTestID == item2.FKTestID);
						if (jobOrderDet != null && jobOrderDet.PriceUnitList.UnitType != 1)
						{
							item2.MinQty = byMasterIDWithSession.FirstOrDefault((JobOrderDetails x) => x.FKTestID == item2.FKTestID).MinQty.Value;
						}

                        JobOrderDetails jobOrderDet1 = byMasterIDWithSession.FirstOrDefault((JobOrderDetails x) => x.FKTestID == item2.FKTestID && x.FKPriceUnitID == item2.FKPriceUnitID);
                        if (!item2.Price.HasValue && jobOrderDet1 != null && jobOrderDet1.Price.HasValue)
						{
                        ...
                        item2.FKGroupID = dbContext.MaterialsDetailsTests.Where(x => x.FKMaterialDetailsID == MaterialDetailsID && x.FKTestID == item2.FKTestID).Select(x => x.FKGroupID).FirstOrDefault();
                    }
                    ...
                    List<SampleReceiveTestList> jobDetail = new List<SampleReceiveTestList>();
                    foreach (JobOrderDetails item in byMasterIDWithSession)
                    {
                        if (allServiceByMaterial.FirstOrDefault((TestsList x) => x.TestID == item.FKTestID) != null)
                        {
                            ...
                            sampleReceiveTestList.FKGroupID = dbContext.MaterialsDetailsTests.Where(x => x.FKMaterialDetailsID == MaterialDetailsID && x.FKTestID == item.FKTestID).Select(x => x.FKGroupID).FirstOrDefault();
                            ...
                            jobDetail.Add(sampleReceiveTestList);
                        }
                    }

                    foreach (SampleReceiveTestList sampleReceiveTestList in jobDetail.OrderByDescending(j => j.FKGroupID))
                    {
                        sampleReceiveTestList.SampleReceiveTestID = ((result.Count > 0) ? (result.Max((SampleReceiveTestList x) => x.SampleReceiveTestID) + 1) : 1);
					...
					foreach (TestsList test in allServiceByMaterial)
					{
                        ...
                        sampleReceiveTestList2.FKGroupID = dbContext.MaterialsDetailsTests.Where(x => x.FKMaterialDetailsID == MaterialDetailsID && x.FKTestID == test.TestID).Select(x => x.FKGroupID).FirstOrDefault();
			...
			if (list.Count > 0)
			{
				...
				if (sampleReceiveTestList != null)
				{
					...
					if (sampleReceiveTestList.FKSampleID == 0 || !sampleReceiveTestList.FKSampleID.HasValue)
					{
						...
						{
                            var testlistls = dbContext.TestsList.Where(x => x.TestID == sampleReceiveTestList.FKTestID).FirstOrDefault();
                            if(testlistls !=null && testlistls.IsReporting !=null && testlistls.IsReporting == true)
                            {
                                sampleReceiveTestList.ReportNumber = GetNewSerial();
                            }
							else
                            {
                                sampleReceiveTestList.ReportNumber = null;
                            }
						}
					}
					else
        ...
        public decimal GetNotSampleReceiveTestsByFKSampleID(long FKSampleID)
        {
            List<ViewSampleReceiveTests> list = ((IQueryable<ViewSampleReceiveTests>)dbContext.ViewSampleReceiveTests).Where((ViewSampleReceiveTests x) =>  x.FKSampleID == (long?)FKSampleID).ToList();
            decimal result = 0m;
            foreach (ViewSampleReceiveTests item in list)
            {
                if (item.Price.HasValue && item.Qty.HasValue)
        ...
        public decimal GetNotSampleReceiveTestsByFKSampleIDAndJobOrderID(long FKSampleID, long JobOrderMasterID)
        {
            var list = ((IQueryable<ViewSampleReceiveTests>)dbContext.ViewSampleReceiveTests).Where((ViewSampleReceiveTests x) => x.FKSampleID == (long?)FKSampleID).Select(P => P.SampleReceiveTestID).ToList();
            decimal result = 0m;
            List<ViewSampleReceiveTests> list1 = ((IQueryable<ViewSampleReceiveTests>)dbContext.ViewSampleReceiveTests).Where((ViewSampleReceiveTests x) => x.JobOrderMasterID == (long?)JobOrderMasterID && x.IsInvoiced != true && x.UnitType != (int?)1).ToList();

            foreach (ViewSampleReceiveTests item in list1)
            {
                if (list.Contains(item.SampleReceiveTestID))
                    continue;
                if (item.Price.HasValue && item.Qty.HasValue)
                {
                    result += item.Price.Value * item.Qty.Value;
        ...
        public decimal GetNotSampleReceiveTestsByJobOrderID(long JobOrderMasterID, List<long> selected)
        {
            List<ViewSampleReceiveTests> list = ((IQueryable<ViewSampleReceiveTests>)dbContext.ViewSampleReceiveTests).Where((ViewSampleReceiveTests x) => x.JobOrderMasterID == (long?)JobOrderMasterID && x.IsInvoiced != true && x.UnitType != (int?)1).ToList();
            decimal result = 0m;
            foreach (ViewSampleReceiveTests item in list)
            {
                if (selected.Contains(item.SampleReceiveTestID))
                    continue;
        ...
        public decimal GetNotSampleReceiveTestsByJobOrderMasterIDTestID(long JobOrderMasterID, List<int> selected)
        {
            List<ViewSampleReceiveTests> list = ((IQueryable<ViewSampleReceiveTests>)dbContext.ViewSampleReceiveTests).Where((ViewSampleReceiveTests x) => x.JobOrderMasterID == (long?)JobOrderMasterID && x.IsInvoiced != true && x.UnitType != (int?)1).ToList();
            decimal result = 0m;
            foreach (ViewSampleReceiveTests item in list)
            {
                if (selected.Contains(item.FKTestID.GetValueOrDefault(0)))
                    continue;
		...
		public int GetNewSerial(int GroupId)
		{
			...
			if (GroupId > 0)
			{
                ...
                List<SampleReceiveTestList> list = source.Where((SampleReceiveTestList x) => x.FKGroupID == GroupId).ToList();
                if (list.Count > 0)
                {
