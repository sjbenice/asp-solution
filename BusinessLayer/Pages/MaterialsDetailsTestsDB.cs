...
namespace BusinessLayer.Pages
{
	public class MaterialsDetailsTestsDB : DBBase<MaterialsDetailsTests, List<MaterialsDetailsTests>, int>
	{
		...
		public List<MaterialsDetailsTests> GetByFKMaterialDetailsIDWithSession(int FKMaterialDetailsID)
		{
			...
			{                
                return ((IQueryable<MaterialsDetailsTests>)dbContext.MaterialsDetailsTests).Where((MaterialsDetailsTests e) => e.FKMaterialDetailsID == FKMaterialDetailsID).OrderBy(m => m.ReportGroup.GroupName ?? "zzz").ThenBy(m => m.TestsList.TestName).ToList();
			}
		...
		public bool UpdateList(List<MaterialsDetailsTests> entityList)
		{
			if (entityList.Count > 0)
			{
				...
				if (fKMaterialDetailsID > 0)
				{
                    ...
                    foreach (MaterialsDetailsTests item3 in entityList)
                    {
                        MaterialsDetailsTests oldMD = CurrentList.Where(c => c.FKTestID == item3.FKTestID).FirstOrDefault();
                        if (oldMD == null)
                            continue;

                        if (oldMD.FKGroupID == item3.FKGroupID)
                            continue;

                        oldMD.FKGroupID = item3.FKGroupID;
                        string message = "";
                        if (!Update(oldMD, out message))
                        {
                            throw new Exception(message);
                        }
                    }
