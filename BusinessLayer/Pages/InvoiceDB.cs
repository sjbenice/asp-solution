    ...
    public class InvoiceDB : DBBase<Invoice, List<Invoice>, long>
	{
		...
		public override bool Delete(Invoice entity, out string message)
		{
            ...
            try
            {
                ...
                {
                    var entityToMove = dbContext.Invoice.Find(entity.InvoiceId); // Retrieve the entity from the first context
                    if (entityToMove != null)
                    {
                        this.dbContext.Invoice.Remove(entityToMove);
                        this.dbContext.Invoice.Attach(entityToMove);
                    }

                    this.dbContext.Entry<Invoice>(entityToMove).State = EntityState.Deleted;
                    if (this.dbContext.SaveChanges() > 0)
                    {
                        result = true;
                    }
        ...
        public List<ViewGelAllInvoice> GetAllFromView()
		{
			return (from x in ((IEnumerable<ViewGelAllInvoice>)dbContext.ViewGelAllInvoice).ToList()
                    where x.NetAmount != 0 && x.NetAmount != null
				orderby x.InvoiceId descending
				select x).ToList();
		}
	}
