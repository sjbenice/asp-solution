...
namespace BusinessLayer.Admin
{
	...
	public class RolesDB
	{
		...
		public bool Delete(ADMRoles entity, out string message)
		{
			...
			try
			{
				IEnumerable<ADMRoleModules> roleModules = this.dbContext.ADMRoleModules.Where(r => r.FKRoleID == entity.RoleID);
                foreach (ADMRoleModules roleModule in roleModules)
                    this.dbContext.Entry<ADMRoleModules>(roleModule).State = EntityState.Deleted;

                IEnumerable<ADMRoleLink> roleLinks = this.dbContext.ADMRoleLink.Where(r => r.FKRoleID == entity.RoleID);
                foreach (ADMRoleLink roleLink in roleLinks)
                {
                    IEnumerable<ADMRoleLinkOptions> roleLinkOptions = this.dbContext.ADMRoleLinkOptions.Where(r => r.FKRoleLinkID == roleLink.RoleLinkID);
                    foreach (ADMRoleLinkOptions roleLinkOption in roleLinkOptions)
                        this.dbContext.Entry<ADMRoleLinkOptions>(roleLinkOption).State = EntityState.Deleted;

                    this.dbContext.Entry<ADMRoleLink>(roleLink).State = EntityState.Deleted;
                }
