...
namespace BusinessLayer.Admin
{
	...
	public class ModuleDB
	{
		...
		public List<ADMModules> GetAllByRoles(List<ADMUserRoles> userRoles)
		{
			...
			{
                RoleModulesDB roleModulesDB = new RoleModulesDB();
                List<ADMRoleModules> roleModules = roleModulesDB.GetByRoleContains(admuserRoles.ADMRoles.RoleID);
                foreach (ADMRoleModules roleModule in roleModules)
                {
                    ADMModules byID = this.GetByID(roleModule.FKModuleID);
