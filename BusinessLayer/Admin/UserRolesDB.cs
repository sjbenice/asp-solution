...
namespace BusinessLayer.Admin
{
	...
	public class UserRolesDB
	{
        ...
        {
            var user = this.dbContext.ADMUsers.First(c => (long)c.UserID == userId);
            if (user.Code.ToLower() == "admin")
                return (from ulo in this.dbContext.UserLinkOptionsView
                        where ulo.Url == PageName
		...
		public List<UserLinkOptionsView> GetAllOptionsForUser(long userId)
		{
            var user = this.dbContext.ADMUsers.First(c => (long)c.UserID == userId);
            if (user.Code.ToLower() == "admin")
                return (from ulo in this.dbContext.UserLinkOptionsView
