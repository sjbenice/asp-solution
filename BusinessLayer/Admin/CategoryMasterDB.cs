...
namespace BusinessLayer.Admin
{
	...
	public class CategoryMasterDB
	{
        ...
        public List<ADMCategoryMaster> GetByRoleId(int roleID)
        {
            return (from CategoryMaster in this.dbContext.ADMCategoryMaster
                    join m in this.dbContext.ADMRoleModules on CategoryMaster.FKModuleId equals m.FKModuleID
                    where m.FKRoleID == roleID
                    select CategoryMaster).ToList<ADMCategoryMaster>();
        }

        public List<ADMCategoryMaster> GetByModuleId(int moduleId)
        {
            return this.dbContext.ADMCategoryMaster.Where(c => c.FKModuleId == moduleId).ToList<ADMCategoryMaster>();
        }
