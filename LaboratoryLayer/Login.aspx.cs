...
public class Login : Page
{
    ...
    protected void loginButton_ServerClick(object sender, EventArgs e)
    {
        ...
        else if (aDMUsers[0].IsRemoved)
        {
            this.lblerror.Text = "User Removed!";
            Invalid = true;
        }
        else if (!aDMUsers[0].IsActive.GetValueOrDefault())
        {
            this.lblerror.Text = "User In-active!";
            Invalid = true;
        }
        else if (aDMUsers[0].ExpiryDate <= DateTime.Now.Date)
        {
            this.lblerror.Text = "User Expired!";
            Invalid = true;
        }
