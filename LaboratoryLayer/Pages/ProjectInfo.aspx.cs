	...
	public class ProjectInfo : Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			if (!base.IsPostBack)
			{
                ...
                Session["ProjectFileAttached"] = null;
        ...
        protected void UploadControl_FileUploadComplete(object sender, FileUploadCompleteEventArgs e)
        {
            ...
            string uploadDir = string.Format(@"{0}{1}", path, "ProjectAttachments");
            ...
            attachedFiles.FileUrl = string.Format(@"localexplorer:{0}", Path.Combine(string.Format(@"{0}{1}", ConfigurationManager.AppSettings["SampleFileReadPath"], "ProjectAttachments"), fileName));
            attachedFiles.IsUrl = false;
            attachedFiles.CreatedDate = DateTime.Now;

            if (Convert.ToInt32(paymentID) == 0 && attachedFiles != null)
            {
                Session["ProjectFileAttached"] = attachedFiles;
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (this.lblAdd.Text == "True")
				{
                    ...
                    AttachedFilesDB attachedFilesDB = new AttachedFilesDB();
                    if (Session["ProjectFileAttached"] != null)
                    {
                        if (Session["ProjectID"] != null)
                        {
                            AttachedFiles attachedFiles = (AttachedFiles)Session["ProjectFileAttached"];
                            attachedFiles.FKTransID = Convert.ToInt32(Session["ProjectID"]);
                            attachedFiles.FKTransTypeID = TransactionTypeID;
        ...
        private int TransactionTypeID=77777;
    }
}
