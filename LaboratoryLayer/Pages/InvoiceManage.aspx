<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Main.Master" CodeBehind="InvoiceManage.aspx.cs" Inherits="LaboratoryLayer.Pages.InvoiceManage" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="body" runat="server">
  ...
  <Header BackColor="SteelBlue" ForeColor="White"></Header>
    ...
    <div class="btn-toolbar">
        <dx:ASPxGridView runat="server" ID="GdvInvoice" AutoGenerateColumns="false" Width="100%" ClientInstanceName="gridInvoice"  
            OnCustomErrorText="GdvInvoice_CustomErrorText" OnBeforeGetCallbackResult="GdvInvoice_BeforeGetCallbackResult"
            DataSourceID="InvoiceDS" KeyFieldName="InvoiceId"  OnCellEditorInitialize="GdvInvoice_CellEditorInitialize" 
            OnCustomButtonInitialize="GdvInvoice_CustomButtonInitialize" OnCommandButtonInitialize="GdvInvoice_CommandButtonInitialize">
          <%--//Note :- Stop All Print out--%>
            <ClientSideEvents CustomButtonClick="function(s, e) { if(e.buttonID=='btnDeleteSR'){ popupDelete.Show(); var key = s.GetRowKey(e.visibleIndex); sid.SetValue(key); }if(e.buttonID=='btnActivateSR'){ popupActivate.Show(); var key = s.GetRowKey(e.visibleIndex); sid.SetValue(key); } if(e.buttonID=='btnView'){ var key = s.GetRowKey(e.visibleIndex); window.open('ReportViwer.aspx?source=Invoice&id=' + key,'Invoice'); window.open('ReportViwer.aspx?source=InvoiceTimeSheetReport&id=' + key,'InvoiceTimeSheetReport'); window.open('ReportViwer.aspx?source=SampleResiveTestInvoiced&id=' + key,'SampleResiveTestInvoiced');}}" />
                ...
                <div style="display: table; margin: 6px 6px 6px auto;">
                    <div style="float:left">
                        <dx:ASPxButton ID="btnSaveDelete" AutoPostBack="true" runat="server" OnClick="btnSaveDelete_Click" Text="Save" CssClass="btn btn-round btn-primary glyphicon glyphicon-floppy-save">
                            <ClientSideEvents Click="function(s, e) {if(txtDeleteReason.GetText()==''){alert('Please provide delete reason');return;} if(!confirm('You can not undo this delete, Are you sure want to delete it')){ txtDeleteReason.SetText(''); } }" />
                        </dx:ASPxButton>
                    </div>
