...
namespace LaboratoryLayer.Pages
{
	...
	{
        public const int TransactionTypeID = 11111;
        ...
        protected void Page_Load(object sender, EventArgs e)
		{
            ...
            if (num > 0)
            {
                ...
                int attachCount = attachedFilesDB.GetAttachmentsCount(num, TransactionTypeID);
                this.lblAttachCount.Text = attachCount > 0 ? attachCount.ToString() : "0";
            }
			...
			if (!base.IsPostBack)
			{
                ...
                flJobOrderMaster.FindItemOrGroupByName("Attachments").Visible = false;
		...
		private JobOrderMaster GetMasterEntity(long MasterID)
		{
			...
			if (num <= 0L)
			{
                ...
                jobOrderMaster.IsReporting = this.IsReporting.Checked;
                jobOrderMaster.Email = txtEmail.Text;
                jobOrderMaster.ContractName = txtContractName.Text;
                jobOrderMaster.ContactNumber = txtContractNumber.Text;
            }
        ...
        protected void cmbFKQuotationMasterID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ProjectsList byIDProject = new ProjectsListDB().GetByID(byID.FKProjectID);

            if (byIDProject.IsClosed)
            {
                cmbFKQuotationMasterID.SelectedIndex = -1;
                this.divmsg.Attributes["class"] = "alert alert-danger";
                this.LblError.ForeColor = Color.Red;
                this.LblError.Text = "the Project " + byIDProject.ProjectName + " of the JO is INACTIVE";
            }
            else
            {
        ...
        protected void btnAddQuotaion_Click(object sender, EventArgs e)
        {
            ...
            var enabledExistingList = list2.Where(s => s.IsEnable).ToList();
            bool exists = false;
            foreach (var jobOrderDetails in enabledExistingList)
            {
                if (list.Where(t => t.FKTestID == jobOrderDetails.FKTestID).Count() > 0)
                {
                    exists = true;
                    break;
                }
            ...
            foreach (JobOrderDetails jobOrderDetails in list2)
            {
                ...
                {
                    if (jobOrderDetails.FKMaterialDetailsID == jobOrderDetails2.FKMaterialDetailsID && jobOrderDetails.FKTestID == jobOrderDetails2.FKTestID)
                    {
                        jobOrderDetails.Price = jobOrderDetails2.Price;
                        jobOrderDetails.MinQty = jobOrderDetails2.MinQty;
                        jobOrderDetails.FKPriceUnitID = jobOrderDetails2.FKPriceUnitID;
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
		{
			...
			{
                int masterId = Convert.ToInt32(base.Request["id"]);
                AttachedFilesDB attachedFilesDB = new AttachedFilesDB();
                int attachCount = attachedFilesDB.GetAttachmentsCount(masterId, TransactionTypeID);
                if (attachCount == 0)
                {
                    this.divmsg.Attributes["class"] = "alert alert-danger";
                    this.LblError.ForeColor = Color.Red;
                    this.LblError.Text = "At least one attachment should be added for Approval.";
                    return;
                }
        ...
        protected ASPxTextBox txtContractName;

        protected ASPxTextBox txtContractNumber;
        ...
        // Token: 0x040002DB RID: 807
        protected ASPxCheckBox IsReporting;

        // Token: 0x040002D7 RID: 808
        protected ASPxLabel lblAttachCount;
    }
}
