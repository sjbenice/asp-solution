	...
	public class SampleReceive : Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!base.IsPostBack)
			{
				...
				this.dtReceiveDate.Date = DateTime.Now;
				this.dtReceiveDate.MinDate = DateTime.Now.AddDays(-4.0);
		...
		private void SetEditMode(long MasterID)
		{
			...
			this.cmbFKMaterialTypeID.ClientEnabled = false;
			this.cmbFKMaterialDetailsID.ClientEnabled = false;
            ...
            if (countSelection == 0)
            {
                this.cmbFKMaterialTypeID.ClientEnabled = true;
                cmbFKMaterialDetailsID.ClientEnabled = true;
		...
		{
			var sampleReceive = new SampleReceiveListDB().GetByID(masterId);
            ...
            string fileName = sampleReceive.SampleNo + "-R" + sampleReceive.RevisionNumber + ".pdf"; // Get the file name
            string fullPath = Path.Combine(folderPath, fileName); // Combine folder path and file name to get the full path
			this.txtSampleNo.Text = sampleReceive.SampleNo;
		...
		private string IsValidData()
		{
			...
			if (num2 == 0L)
			{
                 if(this.dtSamplingDate.Value == null)
                    text += "\n\tPlease Enter Sample date";

                if (this.dtSamplingDate.Date > DateTime.Today.Date)
				text += "\n\tSampling Date should not exceeds current date";
			}
		...
		protected void cmbFKJobOrderMasterID_SelectedIndexChanged(object sender, EventArgs e)
		{
            try
            {
                ...
                ProjectsList byID2 = new ProjectsListDB().GetByID(byID.FKProjectID);
                if (byID2 != null)
                {
                    if (!byID2.IsClosed)
                    {
                    ...
                    {
                        cmbFKJobOrderMasterID.Text = "";
                        cmbFKJobOrderMasterID.SelectedIndex = -1;
                        this.divmsg.Attributes["class"] = "alert alert-danger";
                        this.LblError.ForeColor = Color.Red;
                        this.LblError.Text = "the Project " + byID2.ProjectName + " of the JO is INACTIVE";
                    }
                }
                else
                {
		...
		protected void cmbMaterials_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			this.Session["SampleReceiveTestList"] = null;
            cmbFKMaterialTypeID.ClientEnabled = true;
            cmbFKMaterialDetailsID.ClientEnabled = true;
        }
		...
		protected void OnItemRequestedByValue(object source, ListEditItemRequestedByValueEventArgs e)
		{
			...
			aspxComboBox.DataSource = from x in new SubcontractorsListDB().GetAll().Where(P=>P.IsLocked==false)
		...
		protected void OnItemsRequestedByFilterCondition(object source, ListEditItemsRequestedByFilterConditionEventArgs e)
		{
			...
			List<SubcontractorsList> testContractorsList = new TestContractorsDB().GetTestContractorsListActiveOnly(currentTest);
