...
namespace LaboratoryLayer.Pages
{
    ...
    public class ReportViwer : Page
	{
		...
		protected XtraReport GetReport(string Report, int id, string FilterExpression)
		{
            ...
            {
				QuotationReportFinalNew quotationReportFinal = new QuotationReportFinalNew(id);
            ...
            if (Report == "Invoicee")
            {
                ...
                Invoice.Parameters["PaymentID"].Value = id;
                var paymentmaster=new BusinessLayer.Pages.PaymentMasterDB().GetByID(id);
                if (paymentmaster != null)
                {
                    if (!string.IsNullOrWhiteSpace(paymentmaster.ReceivedBy))
                    {
                        BusinessLayer.ADMUsers admusers = new BusinessLayer.Admin.UsersDB().GetByEName(paymentmaster.ReceivedBy);
                        int userID = admusers.UserID;
                        Invoice.Parameters["FKTransID"].Value = userID;
                        var attahcement = new BusinessLayer.Pages.AttachedFilesDB().GetAttachment(userID, 33333);
                        if (attahcement != null)
                        {
                            try
                            {
                                using (var ms = new System.IO.MemoryStream(attahcement.FileContent))
                                {
                                    Invoice.xrPictureBox1.Image = System.Drawing.Image.FromStream(ms);
                                }
                            }
                            finally
                            {
                    ...
                    if (!string.IsNullOrWhiteSpace(paymentmaster.ApprovedBy))
                    {
                        BusinessLayer.ADMUsers approvedByUsers = new BusinessLayer.Admin.UsersDB().GetByEName(paymentmaster.ApprovedBy);
                        if (approvedByUsers != null)
                        {
                            int approvedByUserID = approvedByUsers.UserID;
                            var approvedByAttached = new BusinessLayer.Pages.AttachedFilesDB().GetAttachment(approvedByUserID, 33333);
                            if (approvedByAttached != null)
                            {
                                try
                                {
                                    using (var ms = new System.IO.MemoryStream(approvedByAttached.FileContent))
                                    {
                                        Invoice.xrPictureBox2.Image = System.Drawing.Image.FromStream(ms);
                                    }
                                }
                                finally { }
                            }
                ...
                this.ReportViewer1.CssClass = "";                                               
                Invoice.CreateDocument();
                return Invoice;
            }
            if (Report == "PaymentManage")
            {
                LaboratoryLayer.Reports.Menu.PaymentManageReport PaymentManage = new LaboratoryLayer.Reports.Menu.PaymentManageReport();                
                PaymentManage.CreateDocument();
            ...
            if (!(Report == "InvoiceList") && !(Report == "Invoice"))
			{
				...
				if (Report == "SampleResiveTestInvoiced")
				{
                    ...
                    //sampleResiveTestInvoiced.Parameters["FilterExpression"].Value = FilterExpression;                    
				...
				if (Report == "RSSReport")
				{
                    ...
                    {
                        rssreport.FilterString = FilterExpression;
