...
namespace LaboratoryLayer.Pages
{
	...
	public class RssInfo : Page
	{
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(text2))
                {
                    ...
                    else if (this.lblAdd.Text == "True")
                    {
                        ...
                        int masterId = Convert.ToInt32(Session["RSSMasterId"]);
                        if (masterId > 0)
                        {
                            GeneratePdfReport(masterId);
        ...
        {
            var rssmaster = new RSSMasterDB().GetByID((long)masterId);

            LaboratoryLayer.Reports.RSSReport rssreport = new LaboratoryLayer.Reports.RSSReport();

            rssreport.Parameters["RSSID"].Value = masterId;

            var virtualPathToSave = "~/Uploaded/RSSReceive/";
            string folderPath = CreateDirectory(virtualPathToSave); // Map the virtual path to a physical path
            string fileName = rssmaster.RSSNumber + "-R" + rssmaster.RevisionNumber + ".pdf"; // Get the file name
            string fullPath = Path.Combine(folderPath, fileName); // Combine folder path and file name to get the full path
                                                                  // this.txtSampleNo.Text = rssmaster.SampleNo;
                                                                  // Export to PDF (you may have your own logic here)
            rssreport.ExportToPdf(fullPath);
        }
        ...
        {
            List<int> selectedTestIDs = this.gdvRSSTestList.GetSelectedFieldValues(new string[]
               {"FkTestId"}).Cast<int>().ToList();

            List<long> updatedList = new List<long>();
            foreach (int val in selectedTestIDs)
                updatedList.Add(val);
            ...
            decimal notSampleReceiveTestsByJobOrderID = new SampleReceiveTestListDB().GetNotSampleReceiveTestsByJobOrderID((long)this.cmbFKJobOrderMasterID.Value, updatedList);
            ...
            decimal num4 = new JobOrderDetailsDB().GetNotReceiveTestsByJobOrderMasterIDTestID((long)this.cmbFKJobOrderMasterID.Value, selectedTestIDs);
            num3 += num4;
