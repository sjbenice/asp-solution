...
namespace LaboratoryLayer.Pages
{
	...
	public class SampleReceivePdfGenerator : Page
	{
        ...
        protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!base.IsPostBack)
			{
			    var sampleReceiveListD = new SampleReceiveListDB();
                var sampleReceiveLists = sampleReceiveListD.GetAll().Count(c => c.RevisionNumber == 0);
                txtRecordNeedReport.Text = sampleReceiveLists + " SampleReceives need generated pdf report";
					...
					if (text2 == "")
					{
						if (num > 0L)
						{
							if (this.lblEdite.Text == "True")
							{
								...
								long masterId = Convert.ToInt64(base.Request["id"]);
								GeneratePdfReport(masterId);
							}
							else
							{
        ...
        private void GeneratePdfReport(long masterId)
		{
			var revisionNumber = new SampleReceiveListDB().GetByID(masterId).RevisionNumber;

			LaboratoryLayer.Reports.SampleReceiptReportNew sampleReceiptReport = new LaboratoryLayer.Reports.SampleReceiptReportNew();
            sampleReceiptReport.Parameters["Id"].Value = masterId;

            var virtualPathToSave = "~/Uploaded/SampleReceive/";
            string folderPath = CreateDirectory(virtualPathToSave); // Map the virtual path to a physical path
			string fileName = txtSampleNo.Text+"-"+ revisionNumber + ".pdf"; // Get the file name
            string fullPath = Path.Combine(folderPath, fileName); // Combine folder path and file name to get the full path
                                                                  // Export to PDF (you may have your own logic here)
            sampleReceiptReport.ExportToPdf(fullPath);
        }

        private string CreateDirectory(string virtualPathToSave)
        {
            var directoryPath = Server.MapPath(virtualPathToSave);
		...
		{
            List<long> selectedTestList = this.gdvSampleTestList.GetSelectedFieldValues(new string[]
               {
                "SampleReceiveTestID"
               }).Cast<long>().ToList();
			...
			decimal notSampleReceiveTestsByJobOrderID = new SampleReceiveTestListDB().GetNotSampleReceiveTestsByJobOrderID((long)this.cmbFKJobOrderMasterID.Value, selectedTestList);
		...
		private string IsValidData()
		{
			...
			list.RemoveAll((SampleReceiveTestList x) => !selectedTestList.ConvertAll<long>(new Converter<object, long>(SampleReceivePdfGenerator.ObjectToLong)).Contains(x.SampleReceiveTestID));
		...
		private SampleReceiveList GetMasterEntity(int MasterID)
		{
            ...
            sampleReceiveList.SampleBroughtInBy = this.txtSampleBroughtInBy.Text;
            sampleReceiveList.SampleLocationIndicatedBy = this.txtSampleLocationIndicatedBy.Text;
		...
		protected void ClearPage()
		{
            ...
            this.txtSampleBroughtInBy.Text = null;
            this.txtSampleLocationIndicatedBy.Text = null;
        ...
        protected void gdvSampleTestList_HtmlDataCellPrepared(object sender, ASPxGridViewTableDataCellEventArgs e)
        {
            if (e.DataColumn.FieldName == "FKGroupID")
            {
                if (e.CellValue != null && (e.CellValue as string) != "")
                {
                    List<object> selectedFieldValues = this.gdvSampleTestList.GetSelectedFieldValues(new string[]
                    {
                        "FKGroupID"
                    });

                    if (selectedFieldValues.Contains(e.CellValue))
                    {
        ...
        // Token: 0x04000290 RID: 656
        protected ASPxTextBox txtSampleBroughtInBy;

        // Token: 0x04000290 RID: 656
        protected ASPxTextBox txtSampleLocationIndicatedBy;
        ...
        protected ASPxTextBox txtCount;
        protected ASPxButton BtnGenerate;
        protected ASPxLabel txtRecordNeedReport;


		protected void BtnGenerate_OnClick(object sender, EventArgs e)
        {
            var targetCount = Convert.ToInt32(string.IsNullOrEmpty(txtCount.Text) ? "0" : txtCount.Text);
            var currentCount = 0;

			var sampleReceiveListD = new SampleReceiveListDB();
            LaboratoryLayer.Reports.SampleReceiptReportNew sampleReceiptReport;
            BtnGenerate.Enabled = false;

			var sampleReceiveLists = sampleReceiveListD.GetAll().Where(c => c.RevisionNumber == 0).ToList();
            foreach (var sampleReceive in sampleReceiveLists)
            {
                if (currentCount >= targetCount)
                {
					...
					this.divmsg.Attributes["class"] = "alert alert-info";
                    this.LblError.Text =
                        currentCount + " report pdf generated from SampleReceipt with Id " +
                        sampleReceiveLists.First().SampleID + " To " + sampleReceiveLists.Skip(currentCount).First().SampleID +
                        " that has No Report";
                    BtnGenerate.Enabled = true;
					return;
				}

				var sampleID = sampleReceive.SampleID;
                var revisionNumber = sampleReceive.RevisionNumber;
                sampleReceiptReport = new LaboratoryLayer.Reports.SampleReceiptReportNew();
                sampleReceiptReport.Parameters["Id"].Value = sampleID;
                var temp = "";
                if (revisionNumber == 0)
                    if (sampleReceiveListD.Update(sampleReceive, out temp))
                        revisionNumber++;

                var virtualPathToSave = "~/Uploaded/SampleReceive/";
                var folderPath = CreateDirectory(virtualPathToSave);
                var fileName = sampleReceive.SampleNo + "-R" + revisionNumber + ".pdf";
