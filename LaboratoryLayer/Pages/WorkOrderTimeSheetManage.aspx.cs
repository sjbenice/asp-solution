	...
	public class WorkOrderTimeSheetManage : Page
	{
		...
		protected void GdvPendingCheckedWorkOrder_CustomButtonInitialize(object sender, ASPxGridViewCustomButtonEventArgs e)
		{
			var roleLinks = new UserRolesDB().GetAllOptionsForUser(int.Parse(this.Session["UserId"].ToString()));
				...
				if (roleLinks.Any(c => c.LinksEName == "Check Time Sheet"))
				{
				...
				else if (!roleLinks.Any(c => c.LinksEName == "Check Time Sheet") && e.ButtonID == "btnChecked")
				{
					e.Enabled = false;
				}
				if (roleLinks.Any(c => c.LinksEName == "Approve Time Sheet"))
				{
				...
				else if (!roleLinks.Any(c => c.LinksEName == "Approve Time Sheet") && e.ButtonID == "btnApprove")
				{
					e.Enabled = false;
				}
