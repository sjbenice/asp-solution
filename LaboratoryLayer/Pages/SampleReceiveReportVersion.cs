...
namespace LaboratoryLayer.Pages
{
    ...
    public class SampleReceiveReportVersion : Page
    {

        protected void Page_Load(object sender, EventArgs e)
        {
            List<UserLinkOptionsView> allOptionsForLink = new UserRolesDB().GetAllOptionsForLink("../Pages/SampleReceiveReportVersion.aspx", long.Parse(this.Session["UserId"].ToString()));
            ...
            if (!base.IsPostBack)
            {
                ...
                if (id > 0L)
                {
                    var SampleNo = new SampleReceiveListDB().GetByID(id).SampleNo;
