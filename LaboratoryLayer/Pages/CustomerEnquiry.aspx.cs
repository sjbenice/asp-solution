...
namespace LaboratoryLayer.Pages
{
	...
	public class CustomerEnquiry : Page
	{
		...
		protected void btnUpdate_Click(object sender, EventArgs e)
		{
            ...
            if (Session["EnquiryDetailsList"] != null)
            {
                List<EnquiryDetails> listEnquiryDetailsList = (Session["EnquiryDetailsList"] as List<EnquiryDetails>);
                foreach (object value4 in selectedFieldValues)
                {
                    if (listEnquiryDetailsList.Where(P => P.FKTestID == Convert.ToInt32(value4)).FirstOrDefault() != null)
                    {
                        this.divmsg.Attributes["class"] = "alert alert-success";
                        this.LblError.ForeColor = Color.Red;
                        this.LblError.Text = "Duplicate TestId Is not Allowed!";
                        this.popTestLists.ShowOnPageLoad = false;
                        GdvLabTestsList.Selection.UnselectAll();
