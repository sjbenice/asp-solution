...
namespace LaboratoryLayer.Pages
{
    ...
    public class AttachmentsReport : Page
    {
        ...
        private void IntitlizeReportAutherization()
        {
            ...
            gdvfiles.Columns[3].Visible = false;
            ...
            foreach (var userLinkOptionsView in distinctOptions)
            {
                switch (userLinkOptionsView.LinksEName)
                {
                    ...
                    case "Test Report Attachements":
                        AssignGridAutherization(gdvfiles, userLinkOptionsView.OptionEName);
                        break;
        ...
        private void AssignGridAutherization(ASPxGridView gridView, string optionEName)
        {
            switch (optionEName)
            {
                ...
                case "Delete":
                    gdvfiles.Columns[3].Visible = true;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!base.IsPostBack)
            {
                ...
                if (Convert.ToInt32(base.Request["TransTypeID"]) == 44444)
                    gdvfiles.Columns["CreatedDate"].Visible = true;
                if (Convert.ToInt32(base.Request["TransTypeID"]) == 66666 || Convert.ToInt32(base.Request["TransTypeID"])==77777 || Convert.ToInt32(base.Request["TransTypeID"]) == 88888)
                {
                    ...
                    gdvfiles.Columns["CreatedDate"].Visible = false;
            }
        }
        ...
        protected void UploadControl_FileUploadComplete(object sender, FileUploadCompleteEventArgs e)
        {
            try
            {
                if (base.Request["TransTypeID"].ToString() == "111222")
                {
                    AttachedFilesDB attachedFilesDB = new AttachedFilesDB();
                    var byID = new TestReportingDB().GetByID(Convert.ToInt32(base.Request["TransID"].ToString()));

                    var samplereceive = new TestReportingDB().GetAll().Where(P => P.TestReportingID == Convert.ToInt32(base.Request["TransID"].ToString())).FirstOrDefault();

                    var reportNumber = samplereceive.ReportNumber;
                    var sampleNumber = samplereceive.SampleNumber;
                    string path = ConfigurationManager.AppSettings["SampleFileReadPath1"];
                    string uploadDir = string.Format(@"{0}{1}\{2}", path, sampleNumber, "ScannedWorksheet");
                    ...
                    string fileName = $"SCD-{reportNumber}-{(attachedFilesDB.CountByReportNumber(reportNumber.ToString(), 111222) + 1).ToString()}{fileExtension}";
                    string fullfilepath = Path.Combine(uploadDir, fileName);

                    // string fileName = e.UploadedFile.FileName;
                    string text = (e.UploadedFile.ContentLength / 1024L).ToString() + " KB";
                    byte[] fileBytes = e.UploadedFile.FileBytes;// new  byte[0]
                    ...
                    attachedFilesDB.Insert(new AttachedFiles
                    {
                        ...
                        FileName = fileName,
                        FileSize = text,
                        FileUrl = string.Format(@"localexplorer:{0}", Path.Combine(string.Format(@"{0}{1}\{2}", ConfigurationManager.AppSettings["SampleFileReadPath"], sampleNumber, "ScannedWorksheet"), fileName)),
                    ...
                    //byID.ScannedLabWS = (string.IsNullOrEmpty(byID.ScannedLabWS) ? fileName : $"{byID.ScannedLabWS},{fileName}"); ;
                    byID.TestEndingDate = DateTime.Now;
            ...
            {
                e.CallbackData = ex.Message + " " + ex.InnerException.ToString();
