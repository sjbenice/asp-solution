...
namespace LaboratoryLayer
{
    ...
    public partial class Main : MasterPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!base.IsPostBack)
            {
                ...
                //if (Convert.ToInt32(base.Session["UserType"]) == 1)
                {
                    ...
                    if (byID.Code.ToLower() == "admin") list3 = new UserRolesDB().GetAll();
                    ModuleDB moduleDB = new ModuleDB();
                    List<ADMModules> modules = moduleDB.GetAllByRoles(list3);
                    if (byID.Code.ToLower() == "admin") modules = moduleDB.GetAll();
                    ...
                    if (linkCategoryByCatMaster.Count > 0)
                    {
                        foreach (ADMLinkCategory admlinkCategory in linkCategoryByCatMaster)
                        {
                            ...
                            if (linksByCategoryforUserRoles.Count > 0)
                            {
                                ...
                                {
                                    if (admlinkCategory.LinkCategoryEName.Trim().ToLower() == "reports" || admlinkCategory.LinkCategoryEName.Trim().ToLower() == "qaqc")
                                    {
        ...
        private void FillSideMenu(List<ADMCategoryMaster> List, List<ADMUserRoles> userList)
        {
            ...
            if (list.Count > 0)
            {
                var notShowTestReport = new[] { 25, 65, 55, 56, 57, 58, 59, 48 };
                var testReportingChild = new[] { 60, 61, 62, 63 };
                var testReportingChildAdded = false;
                    ...
                    if (linkCategoryByCatMaster.Count > 0)
                    {
                        foreach (ADMLinkCategory admlinkCategory in linkCategoryByCatMaster)
                        {
                            ...
                            if (linksByCategoryforUserRoles.Count > 0)
                            {
                                ...
                                foreach (ADMLinks admlinks in linksByCategoryforUserRoles)
                                {
                                    if (notShowTestReport.Contains(admlinks.LinksID))
                                        continue;

                                    if (testReportingChild.Contains(admlinks.LinksID) && testReportingChildAdded)
                                        continue;

                                    if (admlinkCategory.LinkCategoryEName.Trim().ToLower() == "reports" || admlinkCategory.LinkCategoryEName.Trim().ToLower() == "qaqc")
                                    {
                                        stringBuilder.AppendLine(string.Format(
                                            "<li class=''><a href='{0}' target='_blank'><i class='menu-icon fa fa-caret-right'></i>{1}</a><b class='arrow'></b></li>",
                                            admlinks.Url,
                                            testReportingChild.Contains(admlinks.LinksID)
                                                ? "Test Reporting"
                                                : admlinks.LinksEName));
                                    }
                                    else
                                    {
                                        stringBuilder.AppendLine(string.Format("<li class=''><a href='{0}'><i class='menu-icon fa fa-caret-right'></i>{1}</a><b class='arrow'></b></li>", admlinks.Url,
                                            testReportingChild.Contains(admlinks.LinksID)
                                            ? "Test Reporting"
                                            : admlinks.LinksEName));
                                    }

                                    if (testReportingChild.Contains(admlinks.LinksID)) testReportingChildAdded = true;
                                }
                                if (admlinkCategory.OrderNo != 0)
                                {
                                    stringBuilder.AppendLine("</ul></li>");
        ...
        private void FillNotifications()
        {
            ...
            if (!roleLinks.Any(c => c.LinksEName == "Job Order" && c.OptionEName == "View"))
                this.JobOrder.Attributes["class"] = "hidden";

            if (!roleLinks.Any(c => c.LinksEName == "Quotation" && c.OptionEName == "View"))
                this.Quotation.Attributes["class"] = "hidden";

            if (!roleLinks.Any(c => c.LinksEName == "Customer Enquiry" && c.OptionEName == "View"))
                this.Enquiry.Attributes["class"] = "hidden";

            if (!roleLinks.Any(c => c.LinksEName == "Payment Manage" && c.OptionEName == "View"))
                this.UnApprovedPayment.Attributes["class"] = "hidden";

            if (!roleLinks.Any(c => c.LinksEName == "Test Reporting" && c.OptionEName == "View"))
                this.Reporting.Attributes["class"] = "hidden";
            ...
            //        this.Enquiry.Attributes["class"] = "hidden";
            //    }
            //    if (admuserSettings.SettingsName == "Waiting for Approval of Payment" && admuserSettings.SettingsValue != "True")
            //    {
            //        this.UnApprovedPayment.Attributes["class"] = "hidden";
            //    }
            ...
            int countReporting = new TestReportingDB().GetAllPending().Count();
            ...
            this.ReportingNewCount.InnerText = string.Format("You Have {0} New Reporting Enquiry{1}", (countReporting > 0) ? countReporting.ToString() : "No", (countReporting == 0) ? "s" : "");


            if (countReporting > 0)
            {
                this.ReportingCountBadge.Attributes["class"] = "badge";
                this.ReportingCountBadge.InnerText = countReporting.ToString();
            }
            else
            {
                this.ReportingCountBadge.Attributes["class"] = "hidden";
            }

            if (countUnapprovedPayments > 0)
            {
                this.UnApprovedPaymentBadge.Attributes["class"] = "badge";
                this.UnApprovedPaymentBadge.InnerText = countUnapprovedPayments.ToString();
            }
            else
            {
                this.UnApprovedPaymentBadge.Attributes["class"] = "hidden";
            }
