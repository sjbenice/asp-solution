	...
	public class RolesLinkManage : Page
	{
		...
		{			
			if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                checkBox.Enabled = IsModuleAreNotReportOrHistorical();
                ...
                checkBox2.Enabled = IsModuleAreNotReportOrHistorical();
                ...
                checkBox3.Enabled = IsModuleAreNotReportOrHistorical();
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
			{
                ...
                checkBox5.Enabled = IsModuleAreNotReportOrHistorical() && linkId != 25;
				checkBox6.Enabled = IsModuleAreNotReportOrHistorical() && IsNotOldSettingsAdd(linkId) && IsNotOldSettingsAddViewEdit(linkId) && IsNotOldSettingsAddViewDelete(linkId) && linkId != 25;
				checkBox7.Enabled = IsModuleAreNotReportOrHistorical() && IsNotOldSettingsAdd(linkId) && IsNotOldSettingsAddAndView(linkId)  && linkId != 25;
				checkBox8.Enabled = IsNotOldSettingsAdd(linkId) && linkId != 25;
				checkBox9.Enabled = IsModuleAreNotReportOrHistorical() && linkId != 25;
		...
		private	bool IsModuleAreNotReportOrHistorical()
		{
			int moduleId;
			int.TryParse(this.ddlModule.SelectedValue, out moduleId);
			return moduleId != 5 && moduleId != 6;
		}

		private bool IsNotOldSettingsAdd(int linkId)
		{
			var oldSettingsAddId = new int[] { 55, 56, 57, 58, 59, 65 };
			return !oldSettingsAddId.Contains(linkId);
		}
		private bool IsNotOldSettingsAddAndView(int linkId)
		{
			var oldSettingsAddAndViewId = new int[] { 60, 61, 62, 63 };
			return !oldSettingsAddAndViewId.Contains(linkId);
		}

        private bool IsNotOldSettingsAddViewEdit(int linkId)
        {
            var oldSettingsAddAndViewId = new int[] {  61, 62, 63 };
            return !oldSettingsAddAndViewId.Contains(linkId);
        }
        private bool IsNotOldSettingsAddViewDelete(int linkId)
        {
            var oldSettingsAddAndViewId = new int[] { 66 };
            return !oldSettingsAddAndViewId.Contains(linkId);
        }
		...
		private void LoadLinksCategory()
		{
            ...
            this.ddlRoles.DataSource = rolesDB.GetAll();
            this.ddlRoles.DataBind();
            this.ddlRoles.Items.Insert(0, new ListItem("Select", "0"));         
        }
		...
		protected void ddlRoles_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            this.ddlModule.DataSource = categoryMasterDB.GetByRoleId(int.Parse(this.ddlRoles.SelectedValue));
            this.ddlModule.DataBind();
            this.ddlModule.Items.Insert(0, new ListItem("Select", "0"));

			FillLinkCategory(new List<ADMCategoryMaster>());
            BindGrid();
            //this.SetchecksState();
        }

        private void FillLinkCategory(List<ADMCategoryMaster> lists)
		{            
            this.ddlLinkCategory.DataSource = lists;
            this.ddlLinkCategory.DataBind();
            this.ddlLinkCategory.Items.Insert(0, new ListItem("Select", "0"));
        }
